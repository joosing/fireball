...
public class FileUploadProcessorImpl implements FileUploadProcessor {
    ...
    public List<ProtocolMessage> process(String srcPath, @Nullable String dstPath, int chunkSize) {
        ...
        var startChunk = new FileTxChunk(ChunkType.START_OF_FILE, dstPath, srcFile.getPath(), 0, 0);
        ...
        while (remainBytes > 0) {
            ...
            var chunk = new FileTxChunk(ChunkType.FILE_CONTENTS, dstPath, srcFile.getPath(), start, readBytes);
            result.add(chunk);
        ...
        var lastChunk = new FileTxChunk(ChunkType.END_OF_FILE, dstPath, srcFile.getPath(), start, 0);
